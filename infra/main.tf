...
provider "aws" {
  region  = var.region
  profile = "tf-s3tables"  # base creds (your profile)
  assume_role {
    role_arn     = "arn:aws:iam::640168415309:role/Terraform"
    session_name = "tf-aws"
  }
}
...
provider "awscc" {
  ...
  # IMPORTANT: awscc expects an *argument* that's a list of objects
  assume_role = {
    role_arn     = "arn:aws:iam::640168415309:role/Terraform"
    session_name = "tf-awscc"
  }
}

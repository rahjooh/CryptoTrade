...
type DeltaProcessor struct {
	...
	batches   map[string]*models.OrderbookDeltaBatch
...
func NewDeltaProcessor(cfg *appconfig.Config, rawChan <-chan models.RawOrderbookDelta, normChan chan<- models.OrderbookDeltaBatch) *DeltaProcessor {
	...
	return &DeltaProcessor{
		...
		batches:   make(map[string]*models.OrderbookDeltaBatch),
...
func (p *DeltaProcessor) handleMessage(raw models.RawOrderbookDelta) {
...
func (p *DeltaProcessor) addToBatch(raw models.RawOrderbookDelta, entries []models.OrderbookDeltaEntry) {
	p.mu.Lock()
	defer p.mu.Unlock()
	...
	if !ok {
		batch = &models.OrderbookDeltaBatch{
			...
			Symbol:      raw.Symbol,
			Market:      raw.Market,
			Entries:     make([]models.OrderbookDeltaEntry, 0, p.config.Processor.BatchSize),

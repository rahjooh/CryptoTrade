...
//)
//
//func SpotOrderBookSnapshot(ctx context.Context, client *http.Client, symbol string) (*model.SpotOrderBookSnapshot, error) {
...
//}
//
//func SpotOrderBookSnapshotReader(ctx context.Context, client *http.Client, symbols []string, outChan chan<- *model.SpotOrderBookSnapshot) {
...
//					return
//				case <-ticker.C:
//					snapshot, err := SpotOrderBookSnapshot(ctx, client, sym)
//					if err != nil {
//						log.Error().Err(err).Str("symbol", sym).Msg("Failed to fetch snapshot")
//						continue
//					}
//					outChan <- snapshot
//				}
